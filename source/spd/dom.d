...
mixin template DomConvenienceFunctions() {
	...
	body {
		...
		if(e is null)
			throw alloc!ElementNotFoundException(SomeElementType.stringof, "id=" ~ id, file, line);
		return e;
	}
	...
	body {
		...
		if(e is null)
			throw alloc!ElementNotFoundException(SomeElementType.stringof, selector, file, line);
		return e;
	}
	...
	*********************************/
	...
	/// For example: div.addChildren("You can visit my website by ", alloc!Link("mysite.com", "clicking here"), ".");
	...
	*/
	...
	///
	/// Given: < b >cool</ b >, if you call b.wrapIn(alloc!Link("site.com", "my site is "));
...
body {
	auto ret = cast(T) e;
	if(ret is null)
		throw alloc!ElementNotFoundException(T.stringof, "passed value", file, line);
	return ret;
}
...
class Element {
	...
	static Element make(string tagName, string childInfo = null, string childInfo2 = null) {
		...
		switch(tagName) {
			case "#text":
				e = alloc!TextNode(null, childInfo);
				return e;
			// break;
			case "table":
				e = alloc!Table(null);
			...
			break;
			case "tr":
				e = alloc!TableRow(null);
			break;
			case "td", "th":
				e = alloc!TableCell(null, tagName);
			break;
			default:
				e = alloc!Element(null, tagName, null, selfClosed); // parent document should be set elsewhere
		}
	...
	*********************************/
	...
	T getParent(T = Element)(string tagName = null) if(is(T : Element)) {
		...
		static if(!is(T == Element)) {
			auto t = cast(T) par;
			if(t is null)
				throw alloc!ElementNotFoundException("", tagName ~ " parent not found");
		} else
			auto t = par;
...
+/
	...
	@property CssStyle computedStyle() {
		...
		/* done */
			...
			_computedStyle = alloc!CssStyle(null, style); // gives at least something to work with
		}
	...
	*********************************/
	...
	/// swaps one child for a alloc!thing. Returns the old child which is now parentless.
	...
	Element appendText(string text) {
		Element e = alloc!TextNode(parentDocument, text);
		appendChild(e);
		return this;
	}
	...
	Element[] appendHtml(string html) {
		Document d = alloc!Document("<root>" ~ html ~ "</root>");
	...
	*/
	...
	/**
		...
		Returns the alloc!children that replace this.
	*/
	...
	@property void innerRawSource(string rawSource) {
		children.length = 0;
		auto rs = alloc!RawSource(parentDocument, rawSource);
		rs.parentNode = this;
	...
	body {
		...
		throw alloc!Exception("no such child");
	}
	...
	*/
	@property void outerText(string text) {
		parentNode.replaceChild(this, alloc!TextNode(parentDocument, text));
	}
	...
	*/
	@property ElementStream tree() {
		return alloc!ElementStream(this);
	}
...
string htmlEntitiesDecode(string data, bool strict = false) {
	...
	foreach(dchar ch; data) {
		if(tryingEntity) {
			...
			if(ch == '&') {
				if(strict)
					throw alloc!Exception("unterminated entity; & inside another at " ~ to!string(entityBeingTried));
			...
			if(ch == ';') {
				...
				// e.g. you &amp i
				if(strict)
					throw alloc!Exception("unterminated entity at " ~ to!string(entityBeingTried));
				else {
					tryingEntity = false;
...
enum NOGC = false; ~this() {}
	...
	static TextNode fromUndecodedString(Document _parentDocument, string html) {
		auto e = alloc!TextNode(_parentDocument, "");
	...
	override @property Element cloned() {
		auto n = alloc!TextNode(parentDocument, contents);
		return n;
	}
...
class Form : Element {
	...
	/// Otherwise, it makes a alloc!input with type=hidden to keep the value.
	void setValue(string field, string value, bool makeNew) {
		...
		if(eles.length == 0) {
			if(makeNew) {
				...
				throw alloc!Exception("form field does not exist");
		}
		...
		if(eles.length == 1) {
			...
			foreach(e; eles) {
				...
				//if(val is null)
				//	throw alloc!Exception("don't know what to do with radio boxes with null value");
				if(val == value)
					e.setAttribute("checked", "checked");
				else
					e.removeAttribute("checked");
	...
	/// it makes no attempt to find and modify existing elements in the form to the alloc!values.
	...
	/// Adds a alloc!INPUT field to the end of the form with the given attributes.
	Element addInput(string name, string value, string type = "hidden") {
		auto e = alloc!Element(parentDocument, "input", null, true);
		e.name = name;
		e.value = value;
		e.type = type;
...
class Document : FileResource {
	...
	*/
	...
	Utf8Stream handleDataEncoding()(in string rawdata, string dataEncoding, bool strict) {
		...
		if(dataEncoding is null) {
			if(strict)
				throw alloc!MarkupException("I couldn't figure out the encoding of this document.");
			else
	...
	*/
	...
	void parseStream(Utf8Stream data, bool caseSensitive = false, bool strict = false) {
		...
		void parseError(string message) {
			throw alloc!MarkupException(format("char %d (line %d): %s", pos, getLineNumber(pos), message));
		}
		...
		string readTagName() {
				...
				data[pos] != ' ' && data[pos] != '\n' && data[pos] != '\t')
			{
				...
				if(pos == data.length) {
					if(strict)
						throw alloc!Exception("tag name incomplete when file ended");
					else
						break;
				}
			}
		...
		string readAttributeName() {
				...
				data[pos] != ' ' && data[pos] != '\n' && data[pos] != '\t')
			{
				if(data[pos] == '<') {
					if(strict)
						throw alloc!MarkupException("The character < can never appear in an attribute name. Line " ~ to!string(getLineNumber(pos)));
					else
				...
				if(pos == data.length) {
					if(strict)
						throw alloc!Exception("unterminated attribute name");
					else
						break;
				}
			}
		...
		string readAttributeValue() {
			if(pos >= data.length) {
				if(strict)
					throw alloc!Exception("no attribute value before end of file");
				else
					return null;
			}
			switch(data[pos]) {
				...
				case '"':
					...
					if(strict && pos == data.length)
						throw alloc!MarkupException("Unclosed attribute value, started on char " ~ to!string(started));
		...
		RawSource readCDataNode() {
			...
			return alloc!RawSource(this, data[start..pos]);
		}
		...
		Ele readElement(string[] parentChain = null) {
			...
			{
				if(strict) {
					throw alloc!MarkupException("Gone over the input (is there no root element or did it never close?), chain: " ~ to!string(parentChain));
				} else {
			...
			if(pos == data.length) {
				if(strict)
					throw alloc!MarkupException("Found trailing < at end of file");
			...
			switch(data[pos]) {
				...
				case '!': // might be a comment, a doctype, or a special instruction
					...
					if(pos == data.length) {
						if(strict)
							throw alloc!MarkupException("<! opened at end of file");
						...
						if(pos + 3 >= data.length) {
							if(strict)
								throw alloc!MarkupException("unclosed comment");
							end = data.length;
							pos = data.length;
						} else {
							end = pos;
						...
						if(parseSawComment !is null)
							if(parseSawComment(data[commentStart .. end])) {
								return Ele(3, alloc!HtmlComment(this, data[commentStart .. end]), null);
							}
						...
						if(end == -1) {
							if(strict)
								throw alloc!MarkupException("Unclosed CDATA section");
							end = pos;
							cdataEnd = pos;
						} else {
							cdataEnd = pos + end;
						...
						return Ele(0, alloc!TextNode(this, data[cdataStart .. cdataEnd]), null);
					} else {
						auto start = pos;
						while(pos < data.length && data[pos] != '>')
							pos++;
						...
						if(pos == data.length) {
							if(strict)
								throw alloc!MarkupException("unclosed processing instruction (<!xxx>)");
						...
						if(parseSawBangInstruction !is null)
							if(parseSawBangInstruction(data[start .. bangEnds])) {
								...
								return Ele(3, alloc!BangInstruction(this, data[start .. bangEnds]), null);
							}
					}
				...
				case '?':
					...
					*/
					...
					if(pos == data.length) {
						if(strict)
							throw alloc!MarkupException("Unclosed <"~end~" by end of file");
					} else {
						currentIndex++;
				...
				case '/': // closing an element
					...
					if(pos == data.length && data[pos-1] != '>') {
						if(strict)
							throw alloc!MarkupException("File ended before closing tag had a required >");
						else
				...
				default:
					...
					Ele addTag(bool selfClosed) {
						...
						if(tagName == "script" || tagName == "style") {
							if(!selfClosed) {
								...
								if(ending == -1) {
									if(strict)
										throw alloc!Exception("tag " ~ tagName ~ " never closed");
									else {
										...
										if(pos < data.length) {
											e = alloc!TextNode(this, data[pos .. $]);
											pos = data.length;
										}
									}
								} else {
						...
						while(!closed) {
							...
							if(n.type == 3 && n.element !is null) {
								...
								if(n.payload == tagName) // in strict mode, this is always true
									closed = true;
							} else { /*throw alloc!Exception("wtf " ~ tagName);*/ }
						}
						//writef("</%s>\n", tagName);
						return Ele(0, e, null);
					}
					...
					switch(data[pos]) {
						...
						case '\n':
							...
							if(strict && pos >= data.length)
								throw alloc!MarkupException("tag open, didn't find > before end of file");
							...
							switch(data[pos]) {
								...
								default: // it is an attribute
									...
									if(strict && attrName in attributes)
										throw alloc!MarkupException("Repeated attribute: " ~ attrName);

									if(attrName.strip().length)
										attributes[attrName] = attrValue;
									else if(strict) throw alloc!MarkupException("wtf, zero length attribute name");
...
//		return alloc!Element(this, name, null, selfClosed);
	}

	///.
	Element createFragment() {
		return alloc!DocumentFragment(this);
	}

	///.
	Element createTextNode(string content) {
		return alloc!TextNode(this, content);
	}
...
int intFromHex(string hex) {
	...
	for(sizediff_t a = hex.length - 1; a >= 0; a--) {
		...
		if( q >= '0' && q <= '9')
			...
			v = q - 'a' + 10;
		else throw alloc!Exception("Illegal hex character: " ~ q);
...
class CssStyle {
	...
	this(string rule, string content) {
		...
		foreach(part; content.split(";")) {
			...
			if(idx == -1)
				continue;
				//throw alloc!Exception("Bad css rule (no colon): " ~ part);
					...
					switch(c) {
						...
						case '}':
							if(currentThing is &currentValue) {
								rules ~= alloc!CssStyle(currentRule, currentValue);
...
final class Stack(T) {
	...
	void push(T t) {
		if(internalLength >= arr.length) {
			auto oldarr = arr;
			if(arr.length < 4096)
				arr = alloc!(T[])(arr.length * 2);
			else
				arr = alloc!(T[])(arr.length + 4096);
