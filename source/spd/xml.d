...
{
	string pointcpy = src;
	XmlNode root = alloc!XmlNode(null);
	try {
 ...
 * node.addChild(alloc!XmlNode("mynode").setAttribute("x", 50).
 *     addChild(alloc!XmlNode("Waldo").addCData("Hello!"))).write("myfile.xml");
 * --------------------------------*/
class XmlNode
{
	...
	XmlNode addCData(string cdata) {
		auto cd = (_docroot ? _docroot.allocCData : alloc!CData);
		cd.setCData(cdata);
		addChild(cd);
		return this;
	}
	...
	private void parseCloseTag(XmlNode parent, ref string xsrc) {
		...
		if (token.icmp(parent.getName()) != 0) throw alloc!XmlError("Wrong close tag: " ~ token ~ " for parent tag " ~ parent.getName);
	}
	...
	private void parseXMLPI(XmlNode parent, ref string xsrc) {
		...
		if (name[$-1] == '?') {
			...
			name = name[0 .. $-1];
			newnode = (_docroot ? _docroot.allocXmlPI : alloc!XmlPI);
		...
		if (xsrc[0 .. 2] != "?>") throw alloc!XmlError("Could not find the end to xml processing instruction " ~ name);
		xsrc = xsrc[2 .. $];
		parent.addChild(newnode);
	}
	...
	private void parseUCData(XmlNode parent, ref string xsrc) {
		...
		// DO NOT CHANGE THIS TO USE THE CONSTRUCTOR, BECAUSE THE CONSTRUCTOR IS FOR USER USE
		auto cd = (_docroot ? _docroot.allocUCData : alloc!UCData);
		cd._cdata = token;
		parent.addChild(cd);
	}
	...
	private void parseComment(XmlNode parent, ref string xsrc) {
		...
		token = xsrc[0 .. slice];
		xsrc = xsrc[slice+3 .. $];
		auto x = (_docroot ? _docroot.allocXmlComment : alloc!XmlComment);
		x._comment = token;
		parent.addChild(x);
	}
	...
	private void parseOpenTag(XmlNode parent, ref string xsrc, bool preserveWS) {
		...
		auto newnode = (_docroot ? _docroot.allocXmlNode : alloc!XmlNode);
		newnode.setName(name);
		xsrc = stripLeft(xsrc);
		...
		if (xsrc[0] == '/') {
			...
			xsrc = stripLeft(xsrc[1 .. $]);
			// check for >
			if (!xsrc.length || xsrc[0] != '>') throw alloc!XmlError("Unable to find end of " ~ name ~ " tag");
			xsrc = stripLeft(xsrc[1 .. $]);
		...
		if (!ret) throw alloc!XmlError("Missing end tag for " ~ name);
	}
	...
	private string getWSToken(ref string input) {
		...
		input = input[i .. $];
		if (!ret.length) {
			throw alloc!XmlError("Unable to parse token at: " ~ input);
		}
		return ret;
	}
	...
	private void parseAttribute(XmlNode xml, ref string attrstr, string term = null) {
		...
		string ripValue(ref string input) {
			...
			if (x == input.length) {
				throw alloc!XmlError("Missing attribute value terminator for value starting at " ~ input);
			}
		...
		if (!attrstr.length) throw alloc!XmlError("Unexpected end of attribute string near " ~ name);
		if (attrstr[0] != '=') throw alloc!XmlError("Missing = in attribute string with name " ~ name);
		// rip off =
		attrstr = attrstr[1 .. $];
		attrstr = stripLeft(attrstr);
		if (attrstr.length) {
			if (attrstr[0] == '"' || attrstr[0] == '\'') {
				...
				throw alloc!XmlError("Unquoted attribute value for " ~ xml.getName ~ ", starting at: " ~ attrstr);
			}
		} else {
			throw alloc!XmlError("Unexpected end of input for attribute " ~ name ~ " in node " ~ xml.getName);
		}
	...
	XmlNode[] parseXPath(string xpath, bool caseSensitive = false) {
		...
		if (!nextnode.length) {
			...
			if( matchXPathPredicate(nextnode, caseSensitive)) {
				...
				retarr ~= alloc!CData(getAttribute(attr));
			}
	...
	private bool matchXPathPredicate(string predstr, bool caseSen) {
		...
		if (predstr[0] == '[' && predstr[$-1] == ']') {
			...
			throw alloc!XPathError("got malformed predicate match " ~ predstr ~ "\n");
		}
		...
		if (!(predlist.length%2)) throw alloc!XPathError("Encountered a janky predicate: " ~ predstr);
		// verify that odd numbers are "and" or "or"
		foreach (i, pred; predlist) if (i%2 && pred != "and" && pred != "or") {
			throw alloc!XPathError("Encountered consecutive terms not separated by \"and\" or \"or\" starting at: " ~ pred);
		} else if (!(i%2) && (pred == "and" || pred == "or")) {
			throw alloc!XPathError("Encountered consecutive joining terms (\"and\" or \"or\") in: " ~ predstr);
		}
		bool[] res;
		...
		foreach (i, pred; predlist) if (!(i%2)) {
			...
			if (pred.length) {
				...
				if (pred.length > 1 && (pred[0] == '<' || pred[0] == '>' || pred[0] == '!') && pred[1] == '=') {
					...
					throw alloc!XPathError("Could not determine comparator at: " ~ pred);
				}
				if (pred.length < 2 && !isNumeric(pred[0 .. 1])) { 
					throw alloc!XPathError("Badly formed XPath query: Non-numeric comparands must be quoted (" ~ pred ~ ")");
				}
				...
				if (pred[$-1] == '"' && pred[0] == '"') {
					...
					throw alloc!XPathError("Badly formed XPath query: Missing quote (" ~ pred ~ ")");
				}
			}
			elem2 = pred;
		...
		foreach (i, pred; predlist) if (i%2) {
			if (pred == "and") {
				...
				throw alloc!XPathError("Erm...nuh uh");
			}
		}
	...
	private bool compareXPathPredicate(string elem1, string comparator, string elem2, string elem1value, bool caseSen) {
		...
		if (comparator.length && !elem2.length) throw alloc!XPathError("Got a comparator without anything to compare");
		if (comparator.length) {
			bool lres, i1num = isNumeric(elem1value),i2num = isNumeric(elem2);
			if (comparator[0] == '<' || comparator[0] == '>') {
				// Must be numeric
				if (!i2num) {
					throw alloc!XPathError("Badly formed XPath query: comparator '" ~ comparator ~ "' requires a numeric operand Not (" ~ elem2 ~ ")");
				}
				if (!i1num) {
					return false;
				}
	...
	XmlNode opIndexAssign(XmlNode x, int childnum) {
		if (childnum > _children.length) throw alloc!Exception("Child element assignment is outside of array bounds");
...
class CData : XmlNode
{
	...
	override string getName() {
		throw alloc!XmlError("CData nodes do not have names to get.");
	}
	
	/// This throws an exception because CData nodes do not have names.
	override void setName(string newName) {
		throw alloc!XmlError("CData nodes do not have names to set.");
	}
	
	/// This throws an exception because CData nodes do not have attributes.
	override bool hasAttribute(string name) {
		throw alloc!XmlError("CData nodes do not have attributes.");
	}
	
	/// This throws an exception because CData nodes do not have attributes.
	override string getAttribute(string name) {
		throw alloc!XmlError("CData nodes do not have attributes to get.");
	}
	...
	override XmlNode setAttribute(string name, string value) {
		throw alloc!XmlError("CData nodes do not have attributes to set.");
	}
	...
	override XmlNode addChild(XmlNode newNode) {
		throw alloc!XmlError("Cannot add a child node to CData.");
	}
...
class XmlPI : XmlNode {
	...
	override XmlNode addChild(XmlNode newNode) {
		throw alloc!XmlError("Cannot add a child node to XmlPI.");
	}
...
class XmlComment : XmlNode {
	...
	override string getName() {
		throw alloc!XmlError("Comment nodes do not have names to get.");
	}
	
	/// Ditto. (this throws an exception)
	override void setName(string newName) {
		throw alloc!XmlError("Comment nodes do not have names to set.");
	}
	
	/// These events can not be attributed to space monkeys. (this throws an exception)
	override bool hasAttribute(string name) {
		throw alloc!XmlError("Comment nodes do not have attributes.");
	}
	
	/// Ditto. (this throws an exception)
	override string getAttribute(string name) {
		throw alloc!XmlError("Comment nodes do not have attributes to get.");
	}
	...
	/// Ditto. (this throws an exception)
	override XmlNode setAttribute(string name, string value) {
		throw alloc!XmlError("Comment nodes do not have attributes to set.");
	}
	...
	override XmlNode addChild(XmlNode newNode) {
		throw alloc!XmlError("Cannot add a child node to comment.");
	}
...
/** This is the encapsulating class for xml documents that allows reuse of nodes
  ...
  * newdoc = alloc!XmlDocument();
  * newdoc.parse(xmlstring);
  * // and again with the parse constructor
  * newdoc = alloc!XmlDocument(xmlstring);
...
class XmlDocument : XmlNode {
	...
	/// This static opCall should be used when creating alloc!XmlDocuments for use
	static XmlDocument opCall(string constring, bool preserveWS = false) {
		auto root = alloc!XmlDocument;
	...
	XmlNode allocXmlNode() {
		...
		if (xmlNodes.length) {
			...
			// otherwise, allocate a alloc!one and set it up properly
			tmp = alloc!XmlNode();
			tmp._docroot = this;
		}
		return tmp;
