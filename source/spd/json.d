...
public import spd.serialization;
...
struct Json {
	...
	*/
	...
	const {
		...
		if (!op.length) throw alloc!JSONException(format("Got %s, expected %s.", name, expected));
		else throw alloc!JSONException(format("Got %s, expected %s for %s.", name, expected, op));
	}
...
Json serializeToJsonOld(T)(T value)
{
	...
	static if (is(TU == Json)) return value;
		...
		auto ret = alloc!Json[value.length];
...
T deserializeJsonOld(T)(Json src)
{
	...
	static if (is(T == Json)) return src;
		...
		auto dst = alloc!(Unqual!TV[src.length]);
...
/**
	...
	See_Also: spd.serialization.serialize, spd.serialization.deserialize, serializeToJson, deserializeJson
*/
...
private void enforceJson(string file = __FILE__, size_t line = __LINE__)(bool cond, lazy string message = "JSON exception")
{
	...
	else if (!cond) throw alloc!JSONException(message);
}
...
private void enforceJson(string file = __FILE__, size_t line = __LINE__)(bool cond, lazy string message, string err_file, int err_line)
{
	...
	else if (!cond) throw alloc!JSONException(errmsg);
}
